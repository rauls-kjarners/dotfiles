    ...
    keys = {
      -- Symfony Routes picker
      {
        ...
        function()
          ...
          if not ok or type(parsed) ~= "table" then
            vim.notify("Symfony routes: not in a Symfony project or bin/console failed", vim.log.levels.WARN)
            return
          end
      ...
      -- Symfony Services picker
      {
        ...
        function()
          ...
          local ok, parsed = pcall(vim.json.decode, raw)
          if not ok or type(parsed) ~= "table" then
            vim.notify("Symfony services: not in a Symfony project or bin/console failed", vim.log.levels.WARN)
            return
          end
