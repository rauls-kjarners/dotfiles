...
### Prefer several parallel calls over one big prompt
...
- **Carry semantic context forward.** If a delegation follows a `mcp__*__search_symbol` → `mcp__*__get_symbol_info` chain, include the resolved FQN, exact identifier string, and target directories in the prompt — agy starts cold and will re-derive textually, risking the wrong class when method names collide.
...
## Routing: JetBrains MCP vs. agy

Applies only to *consume-and-discard* work (operate-on stays in context — see Core principles) in projects where MCP is exposed. Within consume-and-discard, the split is **semantic vs. textual**:

- **JetBrains MCP** — semantic, bounded queries: single-symbol lookup, single-file or small-set inspections, structural search within a target directory, rename refactors, framework registry lookups, project config introspection. Also the **post-rename `mcp__*__search_text` audit** — even when codebase-wide, it's a single known identifier following a semantic operation, so keep it in MCP to keep the rename context coupled.
...
`mcp__*__search_text` boundary: bounded / single-identifier / post-rename → MCP; wide and exploratory → agy.

When both fit, prefer MCP for precision; switch to agy if the result set will exceed a handful of files.
